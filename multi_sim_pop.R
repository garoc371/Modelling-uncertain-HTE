...
source("microsim_spline/sim_multi_cohort.R")
source("microsim_spline/gen_surface.R")
...
for (i in 1: nrow(combinations)){
  ...
  pop_path <- paste0("microsim_spline/MC_sim/pop_data/",control_short, "_ctrl_", treatment_short, "_trt", sep = "")
  ...
  probs_control <- plogis(control_outcome_logit(baseline_probs,ages_control, control_type))
  probs_treatment <- generate_probabilities(ages_treatment,baseline_probs, control_type, treatment_type)
  probs_control_extra <- plogis(control_outcome_logit(baseline_probs,extra_control, control_type))
  probs_treatment_extra <- generate_probabilities(extra_trt,baseline_probs, control_type, treatment_type)
  
  # Generate the population data
  
  pop_data <- replicate(n = n_rep, expr =
    gen_trial_data(n_trial, n_extra, probs_control, probs_treatment, 
                               probs_control_extra, probs_treatment_extra, ages_control,
                               ages_treatment, extra_control, extra_trt), simplify = F
 )
