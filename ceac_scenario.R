...
for (i in 1:nrow(combinations)) {
  ...
  pp_all_limited[[i]]$models <- setNames(
    vector("list", length(model_types)),
    model_types
  )

  pp_all_extended[[i]] <- list(scenario = scenario_name)
  pp_all_extended[[i]]$models <- setNames(
    vector("list", length(model_types)),
    model_types
  )
  ...
  for (model_type in model_types) {
    ...
    for (j in seq_along(wtp)) {
      pp_temp_limited[j] <- model_scenario_pp(
        scenario_name,
        model_type,
        "_limited",
        wtp[j]
      )
      pp_temp_extended[j] <- model_scenario_pp(
        scenario_name,
        model_type,
        "_extended",
        wtp[j]
...
inmb_positive_wtp <- tibble(
  ...
  wtp_positive = map_dbl(INMB_list_all, function(INMB_vec) {
    wtp[{
      which(INMB_vec > 0)[1]
    }]
  })
...
ggplot(ceac_data_limited, aes(x = wtp, y = probability)) +
  ...
  facet_wrap(
    ~scenario,
    ncol = 4,
    labeller = labeller(scenario = label_wrap_gen(width = 40))
  ) +
  labs(
